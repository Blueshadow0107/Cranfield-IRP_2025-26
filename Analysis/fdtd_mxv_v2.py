...
fdtd_mxv_v2.py
==============
Improved trainable acoustic matrix-vector multiplication benchmark.

Changes from v1:
  - More hard-wall cylinders (default 4).
  - Optimise cylinder positions **and** radii.
  - Use scipy.optimize.differential_evolution instead of random search.
  - Target remains diagonal routing:
        W_target = [[0.8, 0.2],
                    [0.2, 0.8]]

The parameter vector passed to the optimiser is flat:
...
# Design space
N_OBSTACLES = 4
OBSTACLE_X_RANGE = (0.12, 0.40)
OBSTACLE_Y_RANGE = (0.05, 0.27)
OBSTACLE_R_RANGE = (0.015, 0.040)   # radii now variable

# Optimisation control
DE_POPSIZE = 4               # differential evolution population factor
DE_MAXITER = 6               # generations
DE_TOL = 1e-4
DE_SEED = 42
...
class MxVFDTD:
    ...
    def measure_matrix(self, cylinders):
        """
...
def params_to_cylinders(params):
    """Flat parameter vector -> list of (cx, cy, r) tuples."""
    cylinders = []
...
def build_bounds():
    """Return scipy-style bounds [(low, high), ...] for all parameters."""
    bounds = []
    for _ in range(N_OBSTACLES):
        bounds.append(OBSTACLE_X_RANGE)
        bounds.append(OBSTACLE_Y_RANGE)
        bounds.append(OBSTACLE_R_RANGE)
    return bounds


def objective(params):
    """Mean squared error between measured and target matrices."""
    cylinders = params_to_cylinders(params)
    mxv = MxVFDTD()
    W = mxv.measure_matrix(cylinders)
    loss = float(np.mean((W - W_TARGET) ** 2))

    # Global cache for progress reporting
    objective.n_evals += 1
...
def main():
    ...
    print("2x2 acoustic MxV benchmark v2")
    print(f"Grid: {NX}x{NY}, dx={DX*1000:.2f} mm, f={SRC_FREQ/1e3:.1f} kHz")
    print(f"Cylinders: {N_OBSTACLES}, optimising (x, y, r) for each")
    ...
    best_cylinders = params_to_cylinders(result.x)
    ...
    plt.tight_layout()
    plt.savefig(fig_dir / 'mxv_matrix_comparison_v2.png', dpi=150)
    print(f"\nSaved: {fig_dir / 'mxv_matrix_comparison_v2.png'}")

    # -------------------------------------------------------------------------
    # Plot 2: pressure field for best geometry, each source
    # -------------------------------------------------------------------------
    mxv = MxVFDTD()
    fig, axes = plt.subplots(2, 1, figsize=(10, 6))
    for s in range(2):
        solver = mxv._build_solver(best_cylinders)
    ...
    plt.tight_layout()
    plt.savefig(fig_dir / 'mxv_best_field_v2.png', dpi=150)
    print(f"Saved: {fig_dir / 'mxv_best_field_v2.png'}")
