...
# ---------------------------------------------------------------------------
NB = 8                     # blocks per side -> 64 trainable values
REGION_X = (44, 224)       # just downstream of port A .. before probe column
REGION_Y = (64, 176)       # covers A channel band and the full B channel
LAMBDA_TV = 1.0            # see module docstring (calibration)
FAIL_LOSS = 10.0
SEED = 20260720

FIG = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'figures')
PROTOCOL_JSON = os.path.join(FIG, 'rd_train_xor_protocol.json')
EVAL_LOG = os.path.join(FIG, 'rd_train_xor_eval_log.jsonl')
CHECKPOINT_JSON = os.path.join(FIG, 'rd_train_xor_checkpoint.json')
RESULTS_JSON = os.path.join(FIG, 'rd_train_xor_results.json')
...
def compute_protocol():
    ...
    print('[protocol] running 4 uniform-phi0 reference sims '
          f'(u*={US:.7f}) ...', flush=True)
    t0 = time.time()
    ref = {}
    for tag, fa, fb in CASES:
        ref[tag] = run_case(PHI0, fa, fb)
        print(f'[protocol]   case {tag} done '
...
# ---------------------------------------------------------------------------
# Flat-parallel objective (whole population x 4 cases over a process pool)
# ---------------------------------------------------------------------------
_SIM_POOL = None


def _sim_pool():
    """Lazily created pool of 15 sim workers (forkserver).  The DE driver
    itself runs single-process (vectorized=True), so there is no
    nested-daemon problem."""
    global _SIM_POOL
    if _SIM_POOL is None:
        _SIM_POOL = multiprocessing.get_context('forkserver').Pool(15)
    return _SIM_POOL
...
def objective_vectorized(X, proto):
    ...
    Evaluates the population's S x 4 sims as one flat batch across the
    pool, so core utilisation is ~15-way independent of popsize."""
    win_lo, win_hi = proto['window_tu']
    A0 = proto['A0_lone_peak']
    nsteps = proto['nsteps_train']
    S = X.shape[1]
    phis = [build_phi(X[:, j]) for j in range(S)]
    jobs = [(phis[j], fa, fb, nsteps)
            for j in range(S) for (_, fa, fb) in CASES]
    results = _sim_pool().map(_run_case_safe, jobs, chunksize=1)
    losses = np.empty(S)
    for j in range(S):
        rec = {'t': time.time(), 'failed': False}
        pk = {}
        failed = False
        for k, (tag, _, _) in enumerate(CASES):
            data = results[j * len(CASES) + k]
            if data is None:
                failed = True
                break
            pk[tag] = window_peak(data, win_lo, win_hi)
        if failed:
            loss = FAIL_LOSS
            rec.update(failed=True, error='worker failure', loss=FAIL_LOSS)
        else:
            terms = [((pk['10'] / A0) - 1.0) ** 2,
                     ((pk['01'] / A0) - 1.0) ** 2,
                     (pk['00'] / A0) ** 2,
            ...
            tv = tv_term(X[:, j])
            loss = data_term + LAMBDA_TV * tv
            rec.update(peaks=pk, data_term=data_term, tv=tv,
                       loss=float(loss))
        try:
            with open(EVAL_LOG, 'a') as fh:
                fh.write(json.dumps(rec) + '\n')
...
def fig_history(path):
    recs = read_eval_log()
    if not recs:
        print('[fig] no eval log yet -- skipping history figure')
        return
...
def cmd_smoke(args):
    proto = load_protocol()
    print('[smoke] 2 generations, popsize 4 (pipeline validation)')
    out = run_de(proto, popsize=4, maxiter=2, tag='smoke')
    print(f'[smoke] done: nfev={out["nfev"]}, best_loss={out["best_loss"]:.4f}, '
          f'wall={out["wall_s"] / 60:.1f} min')
    print(f'[smoke] per-eval cost ~{out["wall_s"] / out["nfev"]:.2f} s '
          f'(16-way parallel)')
...
def cmd_train(args):
    ...
    out = run_de(proto, popsize=args.popsize, maxiter=args.maxiter,
                 tag='train')
    print(f'[train] done: nfev={out["nfev"]}, '
...
def cmd_robust(args):
    ...
    print('[robust] full-resolution truth table of best design ...',
          flush=True)
    ...
    for tag, fa, fb in CASES:
        ...
        print(f'[robust]   ({tag[0]},{tag[1]}): {trained[tag]:.4f} '
    ...
    print(f'[robust] separation: baseline {sep_ba:.2f}x -> '
          f'trained {sep_tr:.2f}x')

    # (2) timing jitter: dB in {-20, 0, +20} steps
    print('[robust] timing-jitter test ...', flush=True)
    jitter = {}
    for tag, fa, fb in (('10', True, False), ('01', False, True),
        ...
        for dB in (-20, 0, 20):
            ...
            print(f'[robust]   ({tag[0]},{tag[1]}) dB={dB:+d}: '
                  f'peak={pk:.4f}', flush=True)
        jitter[tag] = rows
    ...
    print(f'[robust] saved {RESULTS_JSON}')
    if sep_tr < 3.0:
        print('[robust] SEPARATION < 3x -- XOR-not-found; '
              'fallback demultiplexer campaign indicated '
              '(subcommand demux).')
    else:
        print(f'[robust] XOR FOUND: separation {sep_tr:.2f}x >= 3x')
...
def cmd_demux(args):
    ...
    """
    proto = load_protocol()
    print('[demux] reference runs (uniform phi0) ...', flush=True)
    ref = {}
    ...
    print(f'[demux] tP1={tP1}, tP2={tP2}, A0={A0:.3f}, '
    ...
    print(f'[demux] done: nfev={out["nfev"]}, '
          f'best_loss={out["best_loss"]:.4f}, '
