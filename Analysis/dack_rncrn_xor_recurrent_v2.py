"""
Robust full recurrent Dack-style RNCRN for XOR.

Numerical safeguards:
- sigma_gamma clips large arguments to avoid overflow.
- Output degradation includes a small quadratic term for saturation.
- Tight parameter bounds keep feedback gains moderate.
"""
...
GAMMA = 0.05
DELTA = 1.0
QUAD_DEG = 0.1     # small quadratic degradation: -quad_deg * X3^2
CLIP_Z = 50.0      # clip activation argument
SEED = 0
...
def sigma_gamma(z, gamma=GAMMA):
    """Numerically stable chemical smooth ReLU."""
    zc = np.clip(z, -CLIP_Z, CLIP_Z)
...
def recurrent_ode(t, X3, x1, x2, alpha, omega, theta, beta3):
    ...
    return np.array([beta3 - DELTA*X3[0] - QUAD_DEG*X3[0]**2 + X3[0]*(alpha @ Y)])
...
def train_one(seed=42):
    ...
    p0 = rng.normal(0.0, 0.3, size=N_PERC + 3*N_PERC + N_PERC + 1)
    bounds = [(-1.5, 1.5)] * len(p0)
    result = minimize(
        loss, p0,
        method='L-BFGS-B',
        bounds=bounds,
...
# -----------------------------------------------------------------------------
def plot_results(params, title="Robust recurrent RNCRN solving XOR"):
    ...
    fig.suptitle(title)
    plt.tight_layout()
    plt.savefig('dack_rncrn_xor_recurrent_v2_results.png', dpi=150)
    print("Saved plot to dack_rncrn_xor_recurrent_v2_results.png")
...
if __name__ == '__main__':
    ...
    if p_opt is not None:
        ...
        for x3_init in [0.0, 0.2, 0.5, 1.0, 1.5]:
            sol = solve_ivp(
