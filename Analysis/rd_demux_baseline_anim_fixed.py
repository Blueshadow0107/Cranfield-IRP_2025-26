"""rd_demux_baseline_anim_fixed.py -- baseline demux run (uniform phi0) with VTK
...
def write_vti(path, u, spacing=(1.0, 1.0, 1.0), origin=(0.0, 0.0, 0.0)):
    ...
    # VTK expects x-fastest ordering; u is (nx, ny), so transpose and ravel
...
def run_pattern_anim(tag):
    ...
    rd.set_phi(phi_dark)
    n_flash = int(dm.T_FLASH / dm.DT)
    for s in range(n_flash):
        rd.run(1)
    ...
    write_pvd(
        os.path.join(OUT_DIR, f'pattern_{tag}.pvd'),
        [(t, f'pattern_{tag}/{f}') for t, f in frames]
    )

    # Subsample probe traces to match frames
    return {
        k: [float(v) for v in np.array(vs)[::STRIDE]]
...
def main():
    ...
    with open(os.path.join(OUT_DIR, 'baseline_traces.json'), 'w') as fh:
        json.dump(
            {'dt': dm.DT, 'stride': STRIDE, 'phi0': dm.PHI0, 'patterns': traces},
            fh
        )
