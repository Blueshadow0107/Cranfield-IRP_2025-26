...
% ---------------------------------------------------------------------
\begin{frame}{Graph simulator from measured operators}
...
\begin{itemize}
    ...
    \item \textbf{Edges} = throats with measured transit time $L/c$.
    ...
    \item \textbf{Routing table:} junction topology decides which input can reach which output.
\end{itemize}

\vspace{0.3cm}
This lets us design and score candidate networks in milliseconds before running the full PDE.
...
T-junction with horizontal input A, vertical input B, horizontal output. Dark-spot flashes ($\phi = 0.010 \to 0.002$, 3.0 t.u.) at each port.
...
\vspace{0.3cm}
\textbf{Result:} the graph simulator captures the inhibition operator once the junction routing is encoded. Arrival time is within $\sim 1$ t.u. of the PDE window centre (32.85 t.u.), consistent with the dark-spot launch latency.

\end{frame}

% ---------------------------------------------------------------------
\begin{frame}{When the graph model needs geometry: the XOR leak}

A symmetric Y-junction with a chamber should give XOR, but the PDE says otherwise:
...
\vspace{0.3cm}
\textbf{Why:} extended channel fronts collide in the chamber, but the excitation curves around the annihilation point and re-ignites the output stem. The graph's ideal ``annihilate'' rule is geometry-dependent. We must use the PDE to classify each junction shape before encoding it in the graph.
...
\vspace{0.3cm}
\textbf{Result:} once the local rules are PDE-calibrated, the graph simulator correctly predicts network-level pulse routing.
...
\begin{itemize}
    \item A calibrated catalogue of local operators: conduction speed, refractory time, and collision inhibition window.
...
\begin{itemize}
    ...
    \item Each throat is a 1-D reaction-diffusion cable; each junction is a switch.
    \item The measured transfer functions (wire, filter, inhibition) become rules in an event-driven graph simulator.
    \item PDE validation shows the graph captures A AND (NOT B) and a 2-to-1 priority router, but also that inhibition is geometry-dependent (XOR leak).
