...
Inputs are applied at \emph{ports}: small fixed circular regions of the grid where the light field is temporarily reduced to emit one or more pulses into the surrounding medium, either as a single \texttt{fire} event (one pulse) or as a \texttt{pulse\_train} (a periodic train at a prescribed frequency).
...
All transfer-function experiments of \refSec{sec:meth_experiments} use the dark-spot drive described in \refSec{sec:meth_darkspot}, the experimentally honest encoding for a photosensitive BZ layer; no concentration is ever clamped by hand.
...
The dark-spot drive is a one-shot flash of darkness: a local, temporary reduction of the light field within a circular spot.
...
The input is a dark-spot drive: a circular patch of radius 6 cells is dimmed from the background $\phi = 0.010$ to $\phi = 0.002$ for $T_{\mathrm{flash}} = 3.0$ time units, emitting a single pulse into the surrounding medium.
...
The input port is a circular dark spot of radius 6 cells centred at the left edge of the channel.
...
Input port is a circular dark spot of radius 6 cells at the left edge of the channel; far probe as a 3-cell vertical strip at $x = 240$.
...
The Oregonator domain is $256 \times 200$ cells; input port A is a circular dark spot of radius 6 cells on the horizontal channel, and input port B is a circular dark spot of radius 6 cells on the vertical arm, centred at the junction $x = 96$.
...
The medium is initialised at rest and a circular perturbation of radius $25$ cells is seeded at the centre with $u = 0.8$, $v = 0.2$; this is a one-time initial condition, not a sustained port drive.
...
All characterisation experiments therefore use the same experimentally honest drive, so the measured transfer functions are directly relevant to a photosensitive BZ layer controlled by projected light.
