...
Given the grid-convergence error of approximately $3\%$ discussed in \refSec{sec:meth_uq}, the working-resolution speed is reported as $6.46 \pm 0.19$ cells per time unit.
...
The two-pulse measurement locates the absolute refractory period at $3.0 \pm 0.1$ time units for the Oregonator at candidate A, where the uncertainty reflects the finite time-step and the discrete pulse-interval sampling.
...
The false-output level is set by residual diffraction and numerical noise; repeated measurements with different probe masks vary it by roughly a factor of two, so the separation is reported as $>100\times$ to be conservative.
...
The 3D channel pulse speed is $6.62 \pm 0.20$ cells per time unit, consistent with the 2D value of $6.46 \pm 0.19$ within the grid-convergence uncertainty.
